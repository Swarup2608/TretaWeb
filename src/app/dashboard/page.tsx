...
export default function DashboardPage() {
    ...
    return (
        <div className={`min-h-screen transition-colors duration-300 ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
            ...
            <header className={`shadow-sm animate-fade-in ${theme === 'dark' ? 'bg-gray-800 border-b border-gray-700' : 'bg-white border-b border-gray-200'} sticky top-0 z-50`}>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center py-4">
                        <h1 className="text-2xl font-bold animate-slide-in" style={{ color: 'var(--primary)' }}>
                            Dashboard
                        </h1>
                        <div className="flex items-center gap-6 animate-slide-in animate-delay-100">
                            <span className="text-sm font-medium">Welcome, <span style={{ color: 'var(--primary)' }}>{user?.name}</span></span>
                            ...
                            <button
                                onClick={logout}
                                className="bg-red-600 text-white px-5 py-2 rounded-lg hover:bg-red-700 transition-colors duration-200 shadow-sm font-medium"
                            >
                                Logout
                            </button>
                        </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <div className={`p-6 rounded-xl shadow-lg card-hover animate-fade-in ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} border-l-4`} style={{ borderLeftColor: 'var(--primary)' }}>
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-sm font-medium mb-2 opacity-70">Total Visits</h3>
                                <p className="text-3xl font-bold" style={{ color: 'var(--primary)' }}>{analytics.totalVisits.toLocaleString()}</p>
                            </div>
                    ...
                    <div className={`p-6 rounded-xl shadow-lg card-hover animate-fade-in animate-delay-100 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} border-l-4`} style={{ borderLeftColor: 'var(--secondary)' }}>
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-sm font-medium mb-2 opacity-70">Unique Visitors</h3>
                                <p className="text-3xl font-bold" style={{ color: 'var(--secondary)' }}>{analytics.uniqueVisitors.toLocaleString()}</p>
                            </div>
                    ...
                    <div className={`p-6 rounded-xl shadow-lg card-hover animate-fade-in animate-delay-200 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} border-l-4 border-orange-500`}>
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-sm font-medium mb-2 opacity-70">Bounce Rate</h3>
                                <p className="text-3xl font-bold text-orange-500">{analytics.bounceRate}</p>
                            </div>
                    ...
                    <div className={`p-6 rounded-xl shadow-lg card-hover animate-fade-in animate-delay-300 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} border-l-4`} style={{ borderLeftColor: 'var(--primary)' }}>
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-sm font-medium mb-2 opacity-70">Avg Session</h3>
                                <p className="text-3xl font-bold" style={{ color: 'var(--primary)' }}>{analytics.avgSessionDuration}</p>
                            </div>
                            <div style={{ color: 'var(--primary)' }}>
                                <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 20 20">
